...
contract MockETH8Bit is Permissioned, Ownable2Step {
    ...
    function transfer(address to, inEuint8 calldata encryptedAmount) public returns (bool) {
        transfer(to, FHE.asEuint8(encryptedAmount));
        return true;
    }

    // Transfers an amount from the message sender address to the `to` address.
    function transfer(address to, euint8 amount) public returns (bool) {
    ...
    function EuintbalanceOf(
        address wallet
    ) public view virtual returns (euint8) {
            return balances[wallet];
    }

    // Sets the `encryptedAmount` as the allowance of `spender` over the caller's tokens.
    function approve(address spender, inEuint8 calldata encryptedAmount) public returns (bool) {
        approve(spender, FHE.asEuint8(encryptedAmount));
        return true;
    }

    // Sets the `amount` as the allowance of `spender` over the caller's tokens.
    function approve(address spender, euint8 amount) public returns (bool) {
    ...
    function transferFrom(address from, address to, inEuint8 calldata encryptedAmount) public returns (bool) {
        transferFrom(from, to, FHE.asEuint8(encryptedAmount));
        return true;
    }

    // Transfers `amount` tokens using the caller's allowance.
    function transferFrom(address from, address to, euint8 amount) public returns (bool) {
    ...
    function _updateAllowance(address owner, address spender, euint8 amount) internal returns (ebool) {
    ...
    function _transfer(address from, address to, euint8 amount, ebool isTransferable) internal {
