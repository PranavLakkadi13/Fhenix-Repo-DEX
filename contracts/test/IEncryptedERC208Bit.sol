...
interface IEncryptedERC208bit {
    ...
    function mint(uint8 mintedAmount) external;

    // Transfers an encrypted amount from the message sender address to the `to` address.
    function transfer(address to, inEuint8 calldata encryptedAmount) external returns (bool);
    ...
    function approve(address spender, euint8 amount) external returns (bool) ;
    ...
    function transferFrom(address from, address to, euint8 amount) external returns (bool) ;
